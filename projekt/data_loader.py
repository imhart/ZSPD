...
class DataLoader:
    ...
    def call_add_product(self, product_name, category_id, supplier_id, unit_price, units_in_stock=0, reorder_level=0, product_id=None):
        result = self.cursor.var(str)

        self.cursor.execute("""
            INSERT INTO products (product_id, product_name, category_id, supplier_id, unit_price, units_in_stock, reorder_level)
            VALUES (:1, :2, :3, :4, :5, :6, :7)
        """, (
            product_id,
            product_name,
        ...
        self.connection.commit()
        result.setvalue(0, f"Produkt dodany z product_id={product_id}")
        return result.getvalue()

    def call_delete_product(self, product_id):
        """Wywołanie procedury delete_product"""
        result = self.cursor.var(str)
        self.cursor.callproc('delete_product', [product_id, result])
        self.connection.commit()
        return result.getvalue()
...
if __name__ == "__main__":
    loader = DataLoader()
    try:
        msg = loader.call_add_product('Nowy produkt', 1, 1, 10.99, 100, 1, 123)
